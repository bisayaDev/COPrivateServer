...
namespace Redux
{
    public static class Commands
    {
        ...
        private static void Process_TrojanPack(Player client, string[] command)
        {
            if (client.Account.Permission < PlayerPermission.GM)
                return;

               //uid for CORO,NECK,HRING,CLUB,ARMOR,BLADE,BOOTS,SWORD
            uint[] uids = {118109, 120249, 151249, 480339, 130109, 410339, 160249, 420339};
            ushort[] prof_uids = { 410, 420, 480 };


            for (int i = 0; i < uids.Length; i++)
            {
                var info = Database.ServerDatabase.Context.ItemInformation.GetById(uids[i]);
                var item = new ConquerItem((uint)Common.ItemGenerator.Counter, info);
                item.Plus = 12;
                item.Enchant = 255;
                item.Bless = 7;
            ...
            ushort lvl = 20;
            for (int i = 0; i < prof_uids.Length; i++)
            {
                client.CombatManager.AddOrUpdateProf(prof_uids[i], lvl);
            }

            byte char_level = 140;
            client.SetLevel(char_level);
            client.Experience = 0;

            byte job_id = 15;
            client.Character.Profession = job_id;
            client.Send(new UpdatePacket(client.UID, UpdateType.Profession, client.Character.Profession));
            client.Recalculate();
        ...
        private static void Process_ArcherPack(Player client, string[] command)
        {
            if (client.Account.Permission < PlayerPermission.GM)
                return;

                      //uid for HAT,NECK,RING,BOW,ARMOR,BLADE,BOOTS,SWORD
            uint[] uids = { 113109, 120249, 150249, 500329, 133109, 160249 };
            ushort[] prof_uids = { 500 };
            ...
            client.CombatManager.AddOrUpdateSkill(SkillID.Scatter, 5);
            client.CombatManager.AddOrUpdateSkill(SkillID.Intensify, 3);
            client.CombatManager.AddOrUpdateSkill(SkillID.AdvancedFly, 1);
            client.CombatManager.AddOrUpdateSkill(SkillID.RapidFire, 5);
            client.CombatManager.AddOrUpdateSkill(SkillID.ArrowRain, 0);
            client.CombatManager.AddOrUpdateSkill(SkillID.Cyclone, 0);
            client.CombatManager.AddOrUpdateSkill(SkillID.SpiritHealing, 2);
